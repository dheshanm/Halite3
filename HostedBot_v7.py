...
game.ready("L4TTiCe v7")

# Now that your bot is initialized, save a message to yourself in the log file with some important information.
#   Here, you log here your id, which you can always fetch from the game object by using my_id.
logging.info("Successfully created bot [L4TTiCe] ! My Player ID is {}.".format(game.my_id))
""" <<<Game Loop>>> """
...
while True:
    ...
    if EMERGENCY:
        ...
        for ship in me.get_ships():
            SHIPS[ship.id] = game_map.calculate_distance(ship.position, me.shipyard.position)
        SHIP_LIST = sorted(SHIPS.items(), key=lambda kv: kv[1])
        logging.info("EMERGENCY SHIP LIST : "+str(SHIP_LIST))
        queued_coords = []
        logging.info("Ship Target : " + str(me.shipyard.position))
        for id in SHIP_LIST:
            logging.info("Request Ship #" + str(id))
            ship = me.get_ship(id[0])
            ...
            if predicted_move == []:
                ...
                if next_pos == me.shipyard.position:
                    ...
                    if next_pos not in queued_coords:
                        ...
                        next_pos = ship.position + Position(*Direction.Still)
                        queued_coords.append(next_pos)
